...
- 只需要用户id即可，且用户id存放到了ThreadLocal中

###### 6、查询指定用户的全部地址

- 返回`List<AddressBook>`，与查询默认地址扎不多
...
- 调用以前的查询分类功能将所有分类查询出来
- 对应分类要有对应的口味，因此返回的实体类为新的dto
...
- 前端做了处理，如果是菜品则显示口味规格，如果是套餐则显示加号

- 参数传过来的并没有用户信息，因此要先指定是哪个用户
- 查询当前菜品（忽略了口味，后期考虑优化一下）或者套餐是否已经在购物车中，如果是则直接数量加一，否则添加选择的物品
...
- 向订单明细表插入多条数据（包含多个物品）
    - 可以看出一类物品就是一条明细
    - 在计算总金额时，要转换成BigDecimal进行计算并使用线程安全的atomicInteger自乘
- 清空购物车数据
- 多表需要事务注解
...
- 在服务端注入Redis操作对象
- 发送短信时，将随机生成的验证码缓存到Redis中，并设置有效期为5分钟
- 登录验证时，从Redis中获取缓存的验证码，如果登录成功则删除Redis中的验证码

###### 2、使用Redis缓存菜品数据

- 优化DishController的list方法，先从Redis中获取菜品数据，如果有则直接返回菜品数据，无需查询数据库，如果没有则需要查询数据库，并将查询到的菜品数据放入Redis中
- 优化DishController中的save和update方法，加入清理缓存的逻辑，防止造成的数据不一致性（因此在使用缓存过程中，要注意保持数据库和缓存数据的一致性，如果数据库中数据发生变化，需要及时的清理缓存数据）

###### 3、使用SpringCache整合Redis缓存套餐数据

- .yml文件中配置缓存数据过期时间
- 在启动类上@EnableCaching注解，开启缓存注解功能
- 在SetMealController的list方法上@Cacheable注解，来表示缓存中没有查数据库且返回结果存到缓存中，缓存中有则直接返回结果
- 在SetMealController的save和delete方法上加入@CacheEvict注解，当数据发生改变时，清空缓存，来处理数据库与缓存的一致性问题
